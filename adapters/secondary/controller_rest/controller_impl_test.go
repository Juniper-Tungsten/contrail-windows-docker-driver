...
var _ = Describe("ControllerAdapterImpl", func() {
	...
	Describe("creating new Contrail project", func() {
		var testProject *types.Project

		BeforeEach(func() {
			testProject = NewTestProject(client, DomainName, testTenant)
		})
		It("returns it", func() {
			project, err := client.GetProject(DomainName, testTenant)
			Expect(err).ToNot(HaveOccurred())
			Expect(project.GetUuid()).To(Equal(testProject.GetUuid()))
		})
		It("creates default security group", func() {
			fqn := fmt.Sprintf("%s:%s:%s", DomainName, testTenant, DefaultSecurityGroup)
			secGroup, err := types.SecurityGroupByName(client.ApiClient, fqn)
			Expect(err).ToNot(HaveOccurred())

			prjSec, err := testProject.GetSecurityGroups()
			Expect(err).ToNot(HaveOccurred())
			Expect(secGroup.GetUuid()).To(Equal(prjSec[0].Uuid))
		})
		It("creates default network IPAM", func() {
			fqn := fmt.Sprintf("%s:%s:%s", DomainName, testTenant, DefaultIPAM)
			netIpam, err := types.NetworkIpamByName(client.ApiClient, fqn)
			Expect(err).ToNot(HaveOccurred())

			prjIpams, err := testProject.GetNetworkIpams()
			Expect(err).ToNot(HaveOccurred())
			Expect(netIpam.GetUuid()).To(Equal(prjIpams[0].Uuid))
		})
	})
	...
	Describe("getting or creating Contrail virtual interface", func() {
		...
		Context("when vif doesn't exist in Contrail", func() {
			...
			Context("when default security group doesn't exist", func() {
				BeforeEach(func() {
					RemoveTestSecurityGroup(client.ApiClient, DefaultSecurityGroup, project)
				})
				It("returns an error", func() {
					iface, err := client.GetOrCreateInterface(testNetwork, tenantName, containerID)
					Expect(err).To(HaveOccurred())
